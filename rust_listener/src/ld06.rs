...
pub struct LD06Transport {
    buffer: Box<Vec<u8>>,
    rcv_state: RcvState,
}

impl LD06Transport {
    pub fn new() -> LD06Transport {
        LD06Transport {
            buffer: Box::new(Vec::new()),
            rcv_state: RcvState::WaitStart,
        }
    }

    fn checksum(buffer: &Box<Vec<u8>>) -> u8 {
        let (_, f) = buffer.split_last().unwrap();
    ...
    pub fn put(&mut self, c: u8) -> Option<Box<Vec<u8>>> {
        ...
        match self.rcv_state {
            ...
            RcvState::WaitLen => {
                let nb_points = c as usize & 0x1F;
                let nb_bytes = 3 * nb_points + 8;
